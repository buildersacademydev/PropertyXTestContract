...
(define-public (cancel-listing-ft (listing-id uint) (ft-asset-contract <ft-trait>))
  (let (
    ...
    (try! (as-contract? ((with-ft (contract-of ft-asset-contract) "*" (get amt listing))) 
        (try! (transfer-ft ft-asset-contract (get amt listing) tx-sender maker))
...
(define-public (fulfil-listing-ft-stx (listing-id uint) (ft-asset-contract <ft-trait>) (amt uint))
 (let (
  ...
  (try! (as-contract? ((with-ft (contract-of ft-asset-contract) "*" (get amt listing))) 
    (try! (transfer-ft ft-asset-contract amt tx-sender taker)) ;;transfer
...
(define-public (fulfil-ft-listing-ft
 ...
 (let (
  ...
  ;; Transfer the ft to the purchaser (caller of the function)
  (try! (as-contract? ((with-ft (contract-of ft-asset-contract) "*" (get amt listing))) 
        (try! (transfer-ft ft-asset-contract amt tx-sender taker))
        )
  )
